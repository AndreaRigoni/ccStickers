...
#include "Math/algorithm.h" // are_same
...
void Plot2D::PrintToCsv(std::string file_name, const char sep)
{
    ...
    if(m_curves.empty()) throw std::out_of_range("no curves to plot");
...
void Plot2D::PrintToGnuplotFile(std::string file_name, enum GnuplotStyle style) const
{
           ...
           if(m_curves.empty()) throw std::out_of_range("no curves to plot"); // no curves to plot
           ...
           switch (style) {
           case Plot2D::GnuplotStyle1:
               print_plot_style1(file_name, o);
               break;
           case Plot2D::GnuplotStyle2:
